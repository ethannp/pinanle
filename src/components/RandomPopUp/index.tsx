  ...
  return (
    <Styled.Container>
      ...
      <IoCloseCircleOutline size={40} color={theme.red} style={{position:"absolute", top:"-10", right: "-10", cursor: "pointer"}} onClick={onClose} ></IoCloseCircleOutline>
        ...
        <Styled.Section>
          <p>
          Random Mode is a new way to play Pinanle - instead of starting at the beginning of the piece (classic), a random starting time 
          in the piece will be chosen. Everyone has the same random starting time. 
          </p>
        </Styled.Section>
        <Styled.Section>
          <p>
          Changes will apply to tomorrow&apos;s Pinanle. Today&apos;s Pinanle is still { mode.charAt(0).toUpperCase() + mode.slice(1) } mode.
          </p>
        </Styled.Section>
        <Styled.Spacer />
        <h3 style={{marginTop:"5px"}}>Set Mode</h3>
        <Styled.Section>
          <input type="radio" value="classic" id="classic" name="modeOption" onChange={handleSelectMode} checked={tmrwMode == "classic"}/>
          <label htmlFor="classic">
            <Styled.Option>🏛️ Classic</Styled.Option>
          </label>
          
          <input type="radio" value="random" id="random" name="modeOption" onChange={handleSelectMode} checked={tmrwMode == "random"} />
